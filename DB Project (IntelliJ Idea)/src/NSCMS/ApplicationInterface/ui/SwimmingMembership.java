...
public class SwimmingMembership {
                ...
                if (returnValue == JFileChooser.APPROVE_OPTION) {
                    ...
                    try (FileInputStream fis = new FileInputStream(selectedFile)) {
                        ...
                        try (Connection connection = DriverManager.getConnection(DB_URL, USERNAME, PASSWORD)) {
                            ...
                            if (!paymentStatus) {
                                pstmt = connection.prepareStatement("INSERT INTO project.swimming_membership (paymentstatus, Receipt, Users_userId, payment_date) VALUES (0, ?, ?, ?)");
                                pstmt.setBinaryStream(1, fis, (int) selectedFile.length());
                                pstmt.setInt(2, id);
                                pstmt.setDate(3, currentDate);
                                int rowsAffected = pstmt.executeUpdate();
                                if (rowsAffected > 0) {
                                    String updateQuery = "UPDATE project.swimming_membership SET paymentstatus = 1 WHERE Users_userId = ?";
                            ...
                            if (paymentStatus) {
                                System.out.println("swimmingmonthlypayment");
                                pstmt = connection.prepareStatement("INSERT INTO project.swimmingmonthlypayment (user_ID, paymentstatus) VALUES (?, 1)");
                                pstmt.setInt(1, id);
                                int rowsAffected = pstmt.executeUpdate();
                                if (rowsAffected > 0) {
                                    String updateQuery = "UPDATE project.swimmingmonthlypayment SET paymentstatus = 1 WHERE user_ID = ?";
                                    PreparedStatement updateStmt = connection.prepareStatement(updateQuery);
                                    updateStmt.setInt(1, id);
                                    updateStmt.executeUpdate();
                                    updateStmt.close();
    ...
    private void generateReceipt() {
        ...
        SwimmingInvoice invoice = new SwimmingInvoice();
    ...
    public static void main(String[] args) {
        new SwimmingMembership(464544);
    }
}

class SwimmingInvoice extends JFrame implements Printable {
